...
export function validate(entry, schema) {

  Object.entries(schema).forEach(([key, type]) => {
    const value = entry[key]

    if(!value) {
      return console.warn(`Entry is missing the defined schema key '${key}'`, entry)
    }

    const {valid, expectedType} = checkType(value, type)

    if(!valid) {
      return console.warn(`Key '${key}' is not of type '${expectedType}'`, entry)
    }
  })
